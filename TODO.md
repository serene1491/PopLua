# PopLua 1.0 RC2 Closure

The `1.0.0-rc.2` scope is closed. This file records the release gate; it is not
an open backlog.
...
- [x] Package versions and install snippets use `1.0.0-rc.2`.
...
- `ValueTask` and `ValueTask<T>` are the generated async shapes. `Task` and
  `Task<T>` are rejected.
- `RunInfo`, `Step`, `IRunObserver`, controlled `Load`, inferred async
  functions, and function capabilities are the RC2 host-integration surface.
...
remaining repository TODO committed for `1.0.0-rc.2`.
