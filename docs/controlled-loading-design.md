...
var lua = Engine.Create(b => b.Require((ctx, load) =>
    load.Name == "utils" ? Chunk.Code(source, "module:utils.lua") : null));
```
...
`Load` supplies the validated name, caller, depth, and observer step while the
resolver result stays intentionally small:
...
`IRunObserver` now receives validated loading, loaded/cache-hit, and load-failed
events. Terminal cancellation and quota failures fail any outstanding load
steps together with their owning execution step.
