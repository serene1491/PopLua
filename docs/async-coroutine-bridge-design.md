...
- The generator supports `[Fn]` for module functions and
...
For module methods whose return type is not `ValueTask` or `ValueTask<T>`:

- keep the existing synchronous code path.

For userdata methods with `[Fn]`:
...
For userdata methods whose return type is not `ValueTask` or `ValueTask<T>`:
...
- `[Fn(CountWait = true)]` keeps active-time accounting
...
- Wall-time is the absolute lifetime limit and is never affected by `CountWait`.

Metrics:

- `Metrics.Duration` remains total wall-clock duration.
- `Metrics.Active`, `Metrics.Suspended`, and `Metrics.Suspensions` expose the
  accounting split without requiring hosts to reconstruct it.
...
Suspended/resumed events are useful, but adding them to `IRunObserver` is a
...
- Add generator tests for inferred `ValueTask`, `ValueTask<T>`,
...
- Active-time quota excludes suspended await time by default, and includes it
  when the async generated function sets `CountWait = true`.
...
- Lua active-time quota counts active execution by default, with per-function
  suspended-time policy through `[Fn(CountWait = true)]`. Wall-time quota
