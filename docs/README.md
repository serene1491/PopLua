...
- Async coroutine bridge for source-generated module functions and userdata
  methods, inferred from `ValueTask` / `ValueTask<T>` return types.
- Function-level capabilities through `[Fn(Cap = "...")]`.
- Logical `RunInfo`, per-operation `Step`, composable `IRunObserver` callbacks,
  and active/suspended execution metrics.
...
- Run, quota, capability, and controlled-load observer hooks.
