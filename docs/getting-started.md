...
```bash
dotnet new install PopLua.Templates::1.0.0-rc.2
dotnet new poplua
```
...
Capabilities can also protect one function without hiding its module:

```csharp
[Module("files")]
public partial class FilesModule
{
    [Fn("read", Cap = Caps.FileRead)]
    public static string Read(string name) => ReadApprovedFile(name);
}
```

PopLua checks the capability before entering the host method. This keeps policy
declarations beside the generated API and prevents denied calls from producing
host side effects.
...
`[Context] CancellationToken` injects the current run cancellation directly.
Use `[Text] string` only for display text that should accept Lua scalars or
values with `__tostring`; leave identifiers and storage keys as strict `string`
parameters. C# optional parameters use their declared default when Lua omits
the argument.
...
## 15. Observe Runs And Steps

Give a durable run identity to related sessions and attach one or more
observers to the engine:

```csharp
var observer = new RunLog();
var lua = Engine.Create(builder => builder.Observe(observer));
var run = RunInfo.Create("job-42", attempt: "retry-2");
...
await session.Run(Chunk.Code("return 42", "handler.lua"));
```

Hosts that load a script and dispatch a named entry can call it without
generating another Lua chunk. The cancellation token covers both Lua execution
and suspended host calls:

```csharp
var result = await session.Call("on_message", cancellationToken, Value.From("hello"));
```

```csharp
sealed class RunLog : IRunObserver
{
    public void Started(ScriptContext context, Step step)
        => Console.WriteLine($"{context.Run.Id}: {step.Kind} {step.Name}");

    public void Completed(ScriptContext context, Step step, Metrics metrics)
        => Console.WriteLine($"active={metrics.Active}");
}
```

`RunInfo` correlates retries and multiple sessions; each run, call, and
controlled load has its own `Step`. Observer methods are optional default
interface methods, additive observers are isolated from one another, and
observer failures never change the script result. Metrics distinguish wall,
active, and suspended time.

The controlled module resolver also receives `Load`, including the current load
step, caller, and nesting depth. This supports evidence and policy without
exposing Lua source or making the resolver stateful.

## 16. Generate Lua API Tooling Files
