...
```bash
dotnet add package PopLua --version 1.0.0-rc.2
```
...
`1.0.0-rc.2` has been validated with `dotnet publish -c Release -r
...
- Collect `IRunObserver` events and final `Metrics`.
...
```csharp
var lua = Engine.Create(b => b.Require((ctx, load) =>
    load.Name == "util"
        ? Chunk.Code("return { message = function() return 'ok' end }", "module:util.lua")
        : null));
```

The resolver receives the current `ScriptContext` and a validated `Load` with
the normalized name, caller, depth, and observer step. It returns an approved
`Chunk` or `null` when the module is unavailable. Session
...
```csharp
var lua = Engine.Create(b => b.Observe(new MyDiagnostics()));
```

`IRunObserver` has optional callbacks and receives:

- `Started(ctx, step)` for `Run(...)` and `Call(...)` execution.
- `Completed(ctx, step, metrics)` after successful execution.
- `Failed(ctx, step, error)` after script, quota, sandbox, cancellation, or async
  execution failure.
- `Quota(ctx, step, kind)` before `Failed` when a quota stops execution.
- `Denied(ctx, step, cap)` when a denied capability blocks use.
- `Loading`, `Loaded`, and `LoadFailed` for validated controlled module loads.

`ScriptContext` carries `Identity`, logical `RunInfo`, sandbox, services,
cancellation, and live execution state. `Step` identifies each operation and
its optional parent. `Metrics` carries wall, active, and suspended time,
suspension count, instructions, memory, and call depth. Hosts remain responsible
...
`[Fn(CountWait = true)]`. `wallTime` always counts total
...
declares `[Module(Cap = "...")]` / `[Fn(Cap = "...")]`, or host code calls
`ctx.Sandbox.Require(...)` for dynamic policy decisions.
...
| Active-time quota | Counts active Lua execution and host binding work. Suspended async waits are excluded by default unless the generated function sets `CountWait = true`. Arbitrary synchronous C# can only be stopped at PopLua checkpoints. |
...
- static `[Fn]` methods.
- function-level capabilities through `[Fn(Cap = "...")]`.
- instance `[Fn]` methods with public constructor services resolved from
  `ScriptContext.Services`.
- static `[Const]`.
- stored and computed `[Prop]`.
- `[Context]` as the first parameter for `ScriptContext` or the current
  `CancellationToken`.
- `[Text] string` parameters for opt-in Lua scalar and `__tostring` display
  coercion while ordinary `string` parameters remain strict.
- CLR defaults for omitted optional parameters, including explicit `nil` for
...
- async module functions and userdata instance methods with
  `[Fn]`, `ValueTask`, and `ValueTask<T>`.
- module and userdata `[Fn]` overloads distinguished by Lua-visible arity and
  parameter types.
...
- Optional C# parameters use their declared default when Lua omits the argument.
- Generic Lua-exposed methods/classes are not supported as generated bindings.
- A function may share its Lua name with distinguishable `[Fn]` overloads.
  Non-function name collisions still produce `PLUA008`.
- Overloads must all be synchronous or all be asynchronous. Lua cannot observe
  CLR width distinctions, so pairs such as `int`/`long`, `float`/`double`, or
  two table descriptor types at the same position are indistinguishable.
- Exact Lua types win over broad `Value` and `[Text] string` fallbacks. A call
  that still ties fails explicitly instead of selecting declaration order.
...
| `PLUA012` | `CountWait` is set on a method that does not return `ValueTask` / `ValueTask<T>`. |
| `PLUA013` | `[Text]` is used on a parameter other than `string`. |
| `PLUA014` | `[Context]` is used on a type other than `ScriptContext` or `CancellationToken`. |
| `PLUA015` | Overloads have the same Lua-visible parameter shape. |
| `PLUA016` | One Lua overload group mixes synchronous and asynchronous methods. |
...
Optional parameters are identified in manifests and LuaLS output; their CLR default
value remains source-owned rather than duplicated as manifest data.
Overloads are separate manifest function entries with stable `@1`, `@2`, ...
suffixes and an `overload` ordinal. LuaLS emits one function declaration plus
`---@overload` annotations; Markdown lists every callable signature.
