...
- Added `RunInfo` and per-operation `Step` correlation across `Run`, `Call`,
  function-reference calls, and controlled module loading.
- Added composable `IRunObserver` callbacks for execution, quotas,
  capabilities, and controlled loading. Observer failures are isolated from
  script execution.
- Added active time, suspended time, and suspension count to final `Metrics`.
- Added `[Fn(Cap = "...")]` for function-level capability checks and generated
  manifest, LuaLS, and Markdown metadata.
- Added validated `Load` context to controlled resolvers, including the module
  caller, nesting depth, and correlated load step.
- Added cancellable `Session.Call(..., CancellationToken, ...)` overloads for
  hosts that dispatch named Lua entry points directly.
- Added `[Text]` for opt-in display-text coercion from Lua scalars and values
  with `__tostring`, while ordinary `string` parameters remain strict.
- Added direct `[Context] CancellationToken` injection for generated bindings.
- Added generated `[Fn]` overload dispatch for modules and userdata. Resolution
  uses Lua-visible arity and types, supports sync and async overload groups, and
  is represented in JSON manifests, LuaLS definitions, and Markdown docs.

### Changed

- Async generated bindings are now inferred from `ValueTask` and
  `ValueTask<T>` return types. Removed the redundant `Async` attribute option.
- Removed `PLUA005`; unsupported `Task` / `Task<T>` shapes now use the general
  unsupported-marshaling diagnostic `PLUA002`.
- Replaced the inverted `PauseTime` option with `CountWait`; async waits are
  excluded from active time unless explicitly counted.
- Replaced the single `Diagnostics(...)` sink with additive `Observe(...)`
  registrations and concise default interface callbacks.
- Controlled-loading cache hits now participate in observation without calling
  the host resolver again.
- Updated package examples, templates, API artifacts, and release validation
  for the breaking RC2 contracts.
- Bumped `poplua.api.json` to manifest version 2 for `countWait` and function
  capability metadata.
- Unified module and userdata `[Fn]` analysis in the generator, and unified the
  runtime lifecycle shared by chunks, global calls, and function references.
- Optional C# parameters now use their declared default when omitted by Lua;
  nullable optional parameters also treat explicit `nil` as omitted. Optional
  and text parameters are identified in generated API tooling.
- Indistinguishable overloads and mixed sync/async overload groups now fail at
  compile time instead of depending on declaration order.
...
- Async userdata instance methods through `[Fn]`.
