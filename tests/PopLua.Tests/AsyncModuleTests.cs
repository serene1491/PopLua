...
public sealed class AsyncModuleTests
{
    ...
    [Fact]
    public async Task DefaultWaitDoesNotConsumeActiveTimeWhileSuspended()
    {
        var service = new AsyncApiService();
        var lua = Engine.Create(b => b.Modules<AsyncApiModule, AsyncSecretModule>());
        var services = Services.Create().Add(service);
    ...
    [Fact]
    public async Task CountWaitConsumesActiveTimeWhileSuspended()
    {
        var service = new AsyncApiService();
        var lua = Engine.Create(b => b.Modules<AsyncApiModule, AsyncSecretModule>());
        var services = Services.Create().Add(service);
    ...
    public async Task DiagnosticsCompleteAfterAsyncResume()
    {
        ...
        var lua = Engine.Create(b => b.Modules<AsyncApiModule, AsyncSecretModule>().Observe(diagnostics));
        var services = Services.Create().Add(service);
        ...
        Assert.Equal(1, diagnostics.LastCompletedMetrics.Value.Suspensions);
        Assert.True(diagnostics.LastCompletedMetrics.Value.Suspended > TimeSpan.Zero);
    }
...
public partial class AsyncApiModule(AsyncApiService service)
{
    ...
    [Fn("counted_delay", CountWait = true)]
...
public sealed class RecordingDiagnostics : IRunObserver
{
