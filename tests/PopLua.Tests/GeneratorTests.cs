...
public sealed class GeneratorTests
{
    ...
    [Fact]
    public void IndistinguishableModuleOverloadsReportDiagnostic()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            using PopLua.Context;
        ...
        Assert.Contains(result.Diagnostics, d => d.Id == "PLUA015");
    }

    [Fact]
    public void DistinguishableModuleOverloadsGenerateOneDispatcher()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            ...
            {
                [Fn("find")]
                public static string Find(long id) => id.ToString();
        ...
        Assert.Equal(1, CountOccurrences(generated, "registration.ModuleFunction(Name, \"find\""));
        Assert.Contains("__PopLua_OverloadDispatch", generated, StringComparison.Ordinal);
        Assert.Contains("__PopLua_Find_Overload1", generated, StringComparison.Ordinal);
        Assert.Contains("__PopLua_Find_Overload2", generated, StringComparison.Ordinal);
    }

    [Fact]
    public void MixedSyncAndAsyncOverloadsReportDiagnostic()
    {
        var result = RunGenerator("""
            ...
            public partial class LookupModule
            {
                ...
                [Fn("find")]
                public static ValueTask<string> Find(string name) => ValueTask.FromResult(name);
            }
            """);

        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Id == "PLUA016");
    }
    ...
    [Fact]
    public void TextOnNonStringParameterReportsDiagnostic()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            ...
            {
                [Fn("x")]
                public static long X([Text] long value) => value;
            }
            """);

        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Id == "PLUA013");
    }

    [Fact]
    public void UnsupportedInjectedContextReportsDiagnostic()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            ...
            {
                [Fn("x")]
                public static string X([Context] string value) => value;
            }
            """);

        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Id == "PLUA014");
    }
    ...
    [Fact]
    public void CountWaitWithoutAsyncReportsDiagnostic()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            using PopLua.Context;
    ...
    [Fact]
    public void IndistinguishableUserdataOverloadsReportDiagnostic()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            using PopLua.Context;
    ...
    public void ManifestJsonProviderIsGeneratedWhenEnabled()
    {
        var result = RunGenerator("""
            ...
            public partial class MathModule
            {
                ...
                [Fn("later", Cap = "math.later", CountWait = true)]
        ...
        Assert.Equal("1.0.0-rc.2", root.GetProperty("popluaVersion").GetString());
        ...
        Assert.Equal("math.later", later.GetProperty("capability").GetString());
        Assert.True(later.GetProperty("countWait").GetBoolean());
        ...
        Assert.False(lengthAsync.GetProperty("countWait").GetBoolean());
    ...
    public void LuaLsDefinitionProviderIsGeneratedFromManifestWhenEnabled()
    {
        var result = RunGenerator("""
            ...
            public partial class ApiModule
            {
                ...
                [Fn("fetch", Cap = "api.fetch", CountWait = true)]
        ...
        Assert.Contains("---Requires capability: api.fetch", lua, StringComparison.Ordinal);
    ...
    [Fact]
    public void OverloadsAreRepresentedInEveryGeneratedDocumentationFormat()
    {
        var result = RunGenerator("""
            using PopLua.Binding;
            ...
            public partial class LookupModule
            {
                ...
                /// <summary>Finds an item by name.</summary>
                [Fn("find")]
                public static string Find(string name) => name;
            }
            """, new Dictionary<string, string>
        ...
        var manifestSource = result.GeneratedSources.Single(source => source.HintName == "PopLuaApiManifest.g.cs").SourceText.ToString();
        using var manifest = JsonDocument.Parse(ExtractGeneratedString(manifestSource, "Json"));
        var functions = manifest.RootElement.GetProperty("modules")[0].GetProperty("functions").EnumerateArray().ToArray();
        Assert.Equal(["module:lookup.find@1", "module:lookup.find@2"], functions.Select(function => function.GetProperty("id").GetString()));
        Assert.Equal([1, 2], functions.Select(function => function.GetProperty("overload").GetInt32()));

        var luaLsSource = result.GeneratedSources.Single(source => source.HintName == "PopLuaLuaLsDefinitions.g.cs").SourceText.ToString();
        var luaLs = ExtractGeneratedString(luaLsSource, "Lua");
        Assert.Contains("---@overload fun(name: string): string", luaLs, StringComparison.Ordinal);
        Assert.Equal(1, CountOccurrences(luaLs, "function lookup.find("));

        var docsSource = result.GeneratedSources.Single(source => source.HintName == "PopLuaApiDocs.g.cs").SourceText.ToString();
        var markdown = ExtractGeneratedString(docsSource, "Markdown");
        Assert.Contains("lookup.find(id: integer)", markdown, StringComparison.Ordinal);
        Assert.Contains("lookup.find(name: string)", markdown, StringComparison.Ordinal);
    }
    ...
    public void ApiDocumentationProviderIsGeneratedFromManifestWhenEnabled()
    {
        var result = RunGenerator("""
            ...
            public partial class InventoryModule
            {
                ...
                [Fn("refresh", Cap = "inventory.refresh", CountWait = true)]
        ...
        Assert.Contains("Capability: `inventory.refresh`", markdown, StringComparison.Ordinal);
    ...
    private static int CountOccurrences(string value, string search)
        => Regex.Matches(value, Regex.Escape(search)).Count;
