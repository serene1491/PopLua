...
public sealed class RuntimeTests
{
    ...
    public void RuntimeBuilderRejectsNullServicesAndDiagnostics()
    {
        ...
        Assert.Throws<ArgumentNullException>(() => builder.Observe(null!));
    }
    ...
    public async Task CallEmitsStartedAndFailedDiagnostics()
    {
        ...
        var lua = Engine.Create(b => b.Observe(diagnostics));
        ...
        Assert.Equal("call:missing_function", diagnostics.LastStartedStep?.Name);
        Assert.Equal("plugin-1", diagnostics.LastStartedContext?.Identity.Id);
        Assert.Same(result.Error, diagnostics.LastError);
    }

    [Fact]
    public async Task CallReportsTheActualGlobalType()
    {
        var lua = Engine.Create();

        await using var session = lua.Session(Sandbox.Trusted);
        Assert.True((await session.Run("answer = 42")).Ok);

        var result = await session.Call("answer");

        var error = Assert.IsType<NativeTypeException>(result.Error);
        Assert.False(result.Ok);
        Assert.Equal(ValueKind.Number, error.Actual);
    }

    [Fact]
    public async Task CallAcceptsCancellation()
    {
        var lua = Engine.Create();
        using var cancellation = new CancellationTokenSource();

        await using var session = lua.Session(Sandbox.Trusted);
        Assert.True((await session.Run("function forever() while true do end end")).Ok);
        cancellation.Cancel();

        var result = await session.Call("forever", cancellation.Token);

        Assert.False(result.Ok);
        Assert.Equal("Lua execution canceled.", result.Error?.Message);
    }

    [Fact]
    public async Task RunInfoAndObserversSpanMultipleStepsAndObserverFailuresAreIsolated()
    {
        var recording = new RuntimeRecordingDiagnostics();
        var run = RunInfo.Create("delivery-1", attempt: "attempt-2", parent: "event-3");
        var lua = Engine.Create(b => b
            .Observe(new ThrowingObserver())
            .Observe(recording));

        await using var session = lua.Session(Sandbox.Trusted, run: run);
        Assert.True((await session.Run("function answer() return 42 end")).Ok);
        Assert.Equal(42, (await session.Call<long>("answer")).Unwrap());

        Assert.Equal(2, recording.StartedCount);
        Assert.Same(run, recording.LastStartedContext?.Run);
        Assert.Equal("delivery-1", recording.LastStartedContext?.Run.Id);
        Assert.Equal("attempt-2", recording.LastStartedContext?.Run.Attempt);
    }

    [Fact]
    public async Task FunctionCapabilityIsCheckedBeforeHostCodeRuns()
    {
        CapabilityModule.Calls = 0;
        var diagnostics = new RuntimeRecordingDiagnostics();
        var lua = Engine.Create(b => b.Module<CapabilityModule>().Observe(diagnostics));

        await using var denied = lua.Session(Sandbox.Untrusted);
        var deniedResult = await denied.Run("return mixed.write()");

        Assert.False(deniedResult.Ok);
        Assert.IsType<SandboxException>(deniedResult.Error);
        Assert.Equal(0, CapabilityModule.Calls);

        var allowedSandbox = Sandbox.Build(builder => builder.Allow("data.write"));
        await using var allowed = lua.Session(allowedSandbox);
        Assert.Equal(42, (await allowed.Run<long>("return mixed.write()")).Unwrap());
        Assert.Equal(1, CapabilityModule.Calls);
    }
    ...
    public async Task GcThresholdTracksPeakMemoryAndCallDepthMetrics()
    {
        ...
        Assert.Equal(TimeSpan.Zero, diagnostics.LastCompletedMetrics.Value.Suspended);
        Assert.Equal(0, diagnostics.LastCompletedMetrics.Value.Suspensions);
    }
    ...
    [Module("mixed")]
    public partial class CapabilityModule
    {
        public static int Calls { get; set; }

        [Fn("write", Cap = "data.write")]
    ...
    private sealed class RuntimeRecordingDiagnostics : IRunObserver
    {
        public int StartedCount { get; private set; }
        public int FailedCount { get; private set; }
        public ScriptContext? LastStartedContext { get; private set; }
        public Step? LastStartedStep { get; private set; }
