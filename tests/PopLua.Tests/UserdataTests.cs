...
public sealed class UserdataTests
{
    ...
    [Fact]
    public async Task UserdataMethodOverloadsAreSelectedByArity()
    {
        var lua = Engine.Create(builder => builder.Module<VecModule>());

        await using var session = lua.Session(Sandbox.Trusted);
        var result = await session.Run<string>(
            "local v = vec.new(3, 4); return v:describe() .. ':' .. v:describe('point')");

        Assert.True(result.Ok);
        Assert.Equal("Vec2(3, 4):point=Vec2(3, 4)", result.Unwrap());
    }
...
public partial class Vec2(double x, double y)
{
    ...
    [Fn("describe")]
    public string Describe(string label) => label + "=" + ToString();
