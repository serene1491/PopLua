...
public sealed class GeneratedModuleRuntimeTests
{
    ...
    [Fact]
    public async Task GeneratedModuleSelectsOverloadsByLuaType()
    {
        var lua = Engine.Create(builder => builder.Module<GeneratedMathModule>());

        await using var session = lua.Session(Sandbox.Trusted);
        var result = await session.Run<string>(
            "return mathx.format(42) .. ':' .. mathx.format('macchi') .. ':' .. mathx.format(true)"
            + " .. ':' .. mathx.choice() .. ':' .. mathx.choice('named')");

        Assert.True(result.Ok);
        Assert.Equal("integer:42:string:macchi:value:Bool:empty:named", result.Unwrap());
    }

    [Fact]
    public async Task GeneratedAsyncModuleSelectsOverloadsBeforeInvocation()
    {
        var lua = Engine.Create(builder => builder.Module<AsyncOverloadModule>());

        await using var session = lua.Session(Sandbox.Trusted);
        var result = await session.Run<string>(
            "return async_overload.fetch(7) .. ':' .. async_overload.fetch('lua')");

        Assert.True(result.Ok);
        Assert.Equal("id:7:name:lua", result.Unwrap());
    }

    [Fact]
    public async Task TextParameterUsesUserdataToStringWithoutWeakeningStrings()
    {
        var lua = Engine.Create(b => b.Modules<GeneratedMathModule, VecModule>());

        await using var session = lua.Session(Sandbox.Trusted);
        var text = await session.Run<string>("return mathx.text(vec.new(3, 4))");
        var strict = await session.Run("return mathx.get_user_name(vec.new(3, 4))");
        var table = await session.Run("return mathx.text({})");

        Assert.True(text.Ok);
        Assert.Equal("Vec2(3, 4)", text.Unwrap());
        Assert.False(strict.Ok);
        Assert.Contains("string", strict.Error!.Message, StringComparison.Ordinal);
        Assert.False(table.Ok);
        Assert.Contains("text", table.Error!.Message, StringComparison.Ordinal);
    }

    [Fact]
    public async Task GeneratedModuleUsesOptionalDefaultsAndInjectedCancellation()
    {
        var lua = Engine.Create(b => b.Module<GeneratedMathModule>());

        await using var session = lua.Session(Sandbox.Trusted);
        var result = await session.Run<string>("return mathx.greeting() .. ':' .. mathx.greeting(nil) .. ':' .. tostring(mathx.has_cancellation())");

        Assert.True(result.Ok);
        Assert.Equal("hello world:hello world:true", result.Unwrap());
    }
...
public partial class GeneratedMathModule
{
    ...
    [Fn("text")]
    public static string Text([Text] string value) => value;

    [Fn("greeting")]
    public static string Greeting(string? name = "world") => "hello " + name;

    [Fn("has_cancellation")]
    public static bool HasCancellation([Context] CancellationToken cancellation) =>
    ...
    [Fn("format")]
    public static string Format(long value) => "integer:" + value;

    [Fn("format")]
    public static string Format(string value) => "string:" + value;

    [Fn("format")]
    public static string Format(Value value) => "value:" + value.Kind;

    [Fn("choice")]
    public static string Choice() => "empty";

    [Fn("choice")]
    public static string Choice(string value = "default") => value;
...
[Module("async_overload")]
public partial class AsyncOverloadModule
{
    [Fn("fetch")]
    public static ValueTask<string> Fetch(long id) => ValueTask.FromResult("id:" + id);

    [Fn("fetch")]
    public static ValueTask<string> Fetch(string name) => ValueTask.FromResult("name:" + name);
}
