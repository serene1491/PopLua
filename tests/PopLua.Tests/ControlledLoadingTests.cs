...
public sealed class ControlledLoadingTests
{
    ...
    {
        var resolveCount = 0;
        var lua = Engine.Create(b => b.Require((_, load) =>
        {
            if (load.Name != "util")
                return null;
    ...
    [Fact]
    public async Task ObserverReceivesNestedAndCachedLoadSteps()
    {
        var observer = new LoadObserver();
        var lua = Engine.Create(b => b
            .Observe(observer)
            .Require((_, load) => load.Name switch
            {
                "root" => Chunk.Code("return require('child')", "module:root.lua"),
                "child" => Chunk.Code("return { value = 42 }", "module:child.lua"),
                _ => null,
            }));

        await using var session = lua.Session(Sandbox.Untrusted);
        var result = await session.Run<long>("local a = require('root'); require('child'); return a.value");

        Assert.True(result.Ok);
        Assert.Equal(["root", "child", "child"], observer.Loads.Select(load => load.Name));
        Assert.Null(observer.Loads[0].Caller);
        Assert.Equal(1, observer.Loads[0].Depth);
        Assert.Equal("root", observer.Loads[1].Caller);
        Assert.Equal(2, observer.Loads[1].Depth);
        Assert.Equal([false, false, true], observer.Cached);
        Assert.Empty(observer.Errors);
        Assert.Equal(observer.Loads[0].Step.Id, observer.Loads[1].Step.Parent);
    }

    [Fact]
    public async Task LoadObserverTimeDoesNotConsumeScriptActiveTime()
    {
        var observer = new SlowLoadObserver();
        var lua = Engine.Create(b => b
            .Observe(observer)
            .Require((_, load) => load.Name == "util" ? Chunk.Code("return 42") : null));
        var sandbox = Sandbox.Build(builder => builder.Quota(
            activeTime: TimeSpan.FromMilliseconds(50),
            wallTime: TimeSpan.FromSeconds(2),
            hookInterval: 100));

        await using var session = lua.Session(sandbox);
        var result = await session.Run<long>("return require('util')");
    ...
    [Fact]
    public async Task TerminalCancellationFailsOutstandingLoadStep()
    {
        var observer = new LoadObserver();
        ...
        var lua = Engine.Create(b => b
            ...
            .Observe(observer)
            .Require((_, load) => load.Name == "slow"
                ? Chunk.Code("return async_api.delayed('module')", "module:slow.lua")
                : null));
        using var cancellation = new CancellationTokenSource();

        await using var session = lua.Session(Sandbox.Trusted, services: services);
        var pending = session.Run("return require('slow')", cancellation.Token).AsTask();
        ...
        Assert.Equal("slow", Assert.Single(observer.Loads).Name);
    }
    ...
    public async Task ResolverExceptionFailsClearly()
    {
        var lua = Engine.Create(b => b.Require((ctx, load) =>
        {
            ...
            return Chunk.Code("return true", name: $"module:{load.Name}.lua");
        }));
    ...
    public async Task SessionServiceResolverOverridesRuntimeResolverAndCacheIsPerSession()
    {
        ...
        {
            runtimeCount++;
            return Chunk.Code($"return {{ value = function() return 'runtime:{load.Name}' end }}", name: $"module:{load.Name}.lua");
        }));
        var services = Services.Create().Add<ModuleResolver>((_, load) =>
        {
            serviceCount++;
            return Chunk.Code($"return {{ value = function() return 'service:{load.Name}' end }}", name: $"module:{load.Name}.lua");
        });
    ...
    public async Task RequiredModuleCanSuspendDuringInitialization()
    {
        ...
        var lua = Engine.Create(b => b
            .Module<AsyncApiModule>()
            .Require((_, load) => load.Name == "async_init"
    ...
    private sealed class LoadObserver : IRunObserver
    {
        internal List<Load> Loads { get; } = [];
        internal List<bool> Cached { get; } = [];
        internal List<RuntimeException> Errors { get; } = [];

        public void Loading(ScriptContext ctx, Load load)
            => Loads.Add(load);

        public void Loaded(ScriptContext ctx, Load load, bool cached)
            => Cached.Add(cached);

        public void LoadFailed(ScriptContext ctx, Load load, RuntimeException error)
            => Errors.Add(error);
    }

    private sealed class SlowLoadObserver : IRunObserver
    {
        public void Loading(ScriptContext ctx, Load load)
