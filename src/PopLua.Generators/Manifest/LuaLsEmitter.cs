...
internal static class LuaLsEmitter
{
    ...
    private static void WriteModule(StringBuilder builder, ModuleModel module)
    {
        ...
        foreach (var functions in module.Functions
                     .GroupBy(f => f.Name, StringComparer.Ordinal)
                     .OrderBy(g => g.Key, StringComparer.Ordinal))
            WriteFunctions(builder, module.Name + "." + functions.Key, functions.ToArray());
    }
    ...
    private static void WriteUserdata(StringBuilder builder, UserdataModel userdata)
    {
        ...
        foreach (var methods in userdata.Methods
                     .GroupBy(m => m.Name, StringComparer.Ordinal)
                     .OrderBy(g => g.Key, StringComparer.Ordinal))
            WriteFunctions(builder, userdata.Name + ":" + methods.Key, methods.ToArray());
    }
    ...
    {
        var ordered = functions.OrderBy(f => f.Overload).ToArray();
        var primary = ordered[0];

        WriteSummary(builder, primary.Documentation);
        if (primary.Capability is not null)
            builder.Append("---Requires capability: ").AppendLine(primary.Capability);
        if (primary.IsAsync)
        {
            builder.AppendLine("---@async");
            if (primary.CountWait)
                builder.AppendLine("---Suspended time counts against PopLua active-time quota.");
        }

        foreach (var overload in ordered.Skip(1))
            builder.Append("---@overload fun(")
                .Append(LuaLsParameters(overload))
                .Append(')')
                .Append(LuaLsReturns(overload))
                .AppendLine();

        WriteFunction(builder, qualifiedName, primary, writeDocumentation: false);
    }
    ...
    private static void WriteFunction(
        ...
        bool writeDocumentation = true)
    {
        if (writeDocumentation)
        {
            WriteSummary(builder, function.Documentation);
            if (function.Capability is not null)
                builder.Append("---Requires capability: ").AppendLine(function.Capability);
            if (function.IsAsync)
            {
                builder.AppendLine("---@async");
                if (function.CountWait)
                    builder.AppendLine("---Suspended time counts against PopLua active-time quota.");
            }
        }
        ...
        foreach (var parameter in parameters)
        {
            ...
            else
            {
                ...
                if (parameter.IsOptional)
                    builder.Append('?');
                builder.Append(' ').Append(ToLuaLsType(parameter.Type));
            }
    ...
    private static string LuaLsParameters(FunctionModel function)
        => string.Join(", ", function.Parameters
            .Where(p => !p.IsContext)
            .Select(p => p.IsVariadic
                ? "...: any"
                : p.Name + (p.IsOptional ? "?" : "") + ": " + ToLuaLsType(p.Type)));

    private static string LuaLsReturns(FunctionModel function)
    {
        if (function.Returns.Count == 0)
            return string.Empty;

        return ": " + string.Join(", ", function.Returns.Select(value =>
            value.Type.Kind == ApiTypeKind.ValueArray ? "any..." : ToLuaLsType(value.Type)));
    }
