...
internal static class Diagnostics
{
    ...
    public static readonly DiagnosticDescriptor CountWaitRequiresAsync = new(
        "PLUA012",
        "CountWait requires an async Lua function",
        "[Fn] method '{0}' sets CountWait but does not return ValueTask or ValueTask<T>",
        "PopLua",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    public static readonly DiagnosticDescriptor TextRequiresString = new(
        "PLUA013",
        "Text coercion requires a string parameter",
        "[Text] parameter '{0}' must have type string",
        "PopLua",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    public static readonly DiagnosticDescriptor UnsupportedContextType = new(
        "PLUA014",
        "Unsupported injected context type",
        "[Context] parameter '{0}' must have type ScriptContext or CancellationToken",
        "PopLua",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    public static readonly DiagnosticDescriptor IndistinguishableOverload = new(
        "PLUA015",
        "Lua overloads must be distinguishable",
        "Lua overload '{0}' in '{1}' has the same observable parameter shape as another overload",
        "PopLua",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);

    public static readonly DiagnosticDescriptor MixedAsyncOverload = new(
        "PLUA016",
        "Lua overloads must share one execution mode",
        "Lua overload '{0}' in '{1}' mixes synchronous and asynchronous methods",
        "PopLua",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);
}
