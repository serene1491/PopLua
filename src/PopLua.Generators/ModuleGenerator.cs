...
public sealed class ModuleGenerator : IIncrementalGenerator
{
        ...
        AttributeData userdataAttribute)
    {
        ...
        var valid = true;
        var methods = BuildMethods(context, type, luaName, seenNames, userdata: true, ref valid);
    ...
    private static List<MethodInfo> BuildMethods(
        SourceProductionContext context,
        INamedTypeSymbol type,
        string ownerName,
        ...
        ref bool valid)
    {
        ...
        foreach (var method in type.GetMembers().OfType<IMethodSymbol>())
        {
            ...
            var attribute = GetAttribute(method, "PopLua.Binding.FnAttribute");
            ...
            var luaName = GetConstructorString(attribute) ?? NameHelpers.ToSnakeCase(method.Name);
            var async = TryGetValueTaskResult(method.ReturnType, out var asyncResultType);
            var countWait = GetNamedBool(attribute, "CountWait");
            if (!async && countWait)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    Diagnostics.CountWaitRequiresAsync,
                    method.Locations.FirstOrDefault(),
                    method.Name));
                valid = false;
            }
            ...
            for (var i = 0; i < method.Parameters.Length; i++)
            {
                ...
                var contextParameter = HasAttribute(parameter, "PopLua.Binding.ContextAttribute");
                var textParameter = HasAttribute(parameter, "PopLua.Binding.TextAttribute");
                if (contextParameter && i != 0)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                ...
                if (contextParameter && !IsSupportedContext(parameter.Type))
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Diagnostics.UnsupportedContextType,
                        parameter.Locations.FirstOrDefault(),
                        parameter.Name));
                    valid = false;
                }

                if (textParameter && TypeKey(parameter.Type) != "string")
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Diagnostics.TextRequiresString,
                        parameter.Locations.FirstOrDefault(),
                        parameter.Name));
                    valid = false;
                }
                ...
                if (!contextParameter && !IsSupportedParameter(parameter.Type))
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Diagnostics.UnsupportedType,
                        parameter.Locations.FirstOrDefault(),
            ...
            methods.Add(new MethodInfo(
                ...
                async,
                countWait,
                GetNamedString(attribute, "Cap")));
        }

        foreach (var symbolGroup in methods.GroupBy(method => method.SymbolName, StringComparer.Ordinal))
        {
            var sameName = symbolGroup.ToArray();
            for (var i = 0; i < sameName.Length; i++)
                sameName[i].BindingName = sameName.Length == 1
                    ? sameName[i].SymbolName
                    : sameName[i].SymbolName + "_Overload" + (i + 1).ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        var overloadGroup = 0;
        foreach (var group in methods.GroupBy(method => method.LuaName, StringComparer.Ordinal))
        {
            ...
            if (!seenNames.Add(group.Key))
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    Diagnostics.DuplicateLuaName,
                    overloads[0].Symbol.Locations.FirstOrDefault(),
                    group.Key,
                    ownerName));
                valid = false;
            }
            ...
            {
                overloads[i].Overload = overloads.Length > 1 ? i + 1 : 0;
                overloads[i].DispatcherName = overloads.Length > 1
                    ? "OverloadDispatch" + overloadGroup.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : overloads[i].BindingName;
            }
            ...
            if (overloads.Any(method => method.IsAsync != overloads[0].IsAsync))
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    Diagnostics.MixedAsyncOverload,
                    overloads[1].Symbol.Locations.FirstOrDefault(),
                    group.Key,
                    ownerName));
                valid = false;
            }

            foreach (var duplicateShape in overloads
                         .GroupBy(OverloadShape, StringComparer.Ordinal)
                         .Where(shape => shape.Count() > 1))
            {
                foreach (var method in duplicateShape.Skip(1))
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Diagnostics.IndistinguishableOverload,
    ...
    private static string OverloadShape(MethodInfo method)
        => string.Join(",", method.Parameters
            .Where(parameter => !parameter.IsContext)
            .Select(parameter =>
                OverloadType(parameter)
                + (parameter.HasDefault ? "?" : "")
                + (IsValueArray(parameter.Type) ? "*" : "")));

    private static string OverloadType(ParameterInfo parameter)
    {
        if (parameter.IsText)
            return "text";

        var key = TypeKey(parameter.Type);
        return key switch
        {
            "int" or "uint" or "long" or "ulong" => "integer",
            "float" or "double" => "number",
            "Descriptor" or "DescriptorList" or "StringList" => "table",
            "Userdata" => "userdata:" + parameter.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat),
            _ => key,
        };
    }
        ...
        AttributeData moduleAttribute)
    {
        ...
        var seenNames = new HashSet<string>(StringComparer.Ordinal);
        var valid = true;
        var methods = BuildMethods(context, type, moduleName, seenNames, userdata: false, ref valid);
        ...
        IReadOnlyList<INamedTypeSymbol> descriptors)
    {
        ...
        foreach (var method in methods.GroupBy(method => method.LuaName, StringComparer.Ordinal).Select(group => group.First()))
            builder.Append("    private static readonly byte[] __PopLua_Key_")
                .Append(method.DispatcherName)
                .Append(" = ")
        ...
        foreach (var group in methods.GroupBy(method => method.LuaName, StringComparer.Ordinal))
        {
            var overloads = group.ToArray();
            if (overloads.Length > 1)
                AppendOverloadDispatcher(builder, overloads, argumentOffset: 1);

            foreach (var method in overloads)
                AppendUserdataMethod(builder, fqType, method, unmanaged: overloads.Length == 1);
        }
    ...
    private static void AppendUserdataMethod(
        ...
        bool unmanaged)
    {
        ...
        if (unmanaged)
            builder.AppendLine("    [UnmanagedCallersOnly(CallConvs = new[] { typeof(CallConvCdecl) })]");
        builder.Append("    private static int __PopLua_").Append(method.BindingName).AppendLine("(nint state)");
        builder.AppendLine("    {");
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        if (method.Cap is not null)
        {
            builder.Append("            global::PopLua.Binding.Marshaller.Context(state).Sandbox.Require(\"")
                .Append(NameHelpers.Escape(method.Cap))
                .AppendLine("\");");
        }
        builder.Append("            var __poplua_self = global::PopLua.Binding.Marshaller.ReadUserdata<").Append(fqType)
            .AppendLine(">(state, 1, __PopLua_MetatableName);");
        ...
        foreach (var parameter in method.Parameters)
        {
            if (parameter.IsContext)
            {
                builder.Append("            var ").Append(parameter.Name).Append(" = ")
                    .Append(ContextExpression(parameter.Type, "global::PopLua.Binding.Marshaller.Context(state)"))
                    .AppendLine(";");
            }
            else
            {
                builder.Append("            var ").Append(parameter.Name).Append(" = ")
                    .Append(ReadParameterExpression(parameter, luaIndex)).AppendLine(";");
        ...
        if (method.IsAsync)
        {
            ...
            if (method.AsyncResultType is null)
            {
                ...
                builder.Append("            return global::PopLua.Binding.Marshaller.BeginAsync(state, __poplua_task, pauseTime: ")
                    .Append(method.CountWait ? "false" : "true")
                    .AppendLine(");");
            }
            else
            {
        ...
        IReadOnlyList<INamedTypeSymbol> descriptors)
    {
        ...
        foreach (var group in methods.GroupBy(method => method.LuaName, StringComparer.Ordinal))
        {
            var overloads = group.ToArray();
            if (overloads.Length > 1)
                AppendOverloadDispatcher(builder, overloads, argumentOffset: 0);
            ...
            foreach (var parameter in method.Parameters)
            {
                if (parameter.IsContext)
                {
                    if (method.IsStatic)
                        ...
                        builder.Append("            var ").Append(parameter.Name).Append(" = ")
                            .Append(ContextExpression(parameter.Type, "__poplua_ctx"))
                            .AppendLine(";");
                }
                else
                {
    ...
    private static void AppendOverloadDispatcher(
        StringBuilder builder,
        IReadOnlyList<MethodInfo> methods,
        int argumentOffset)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("    [UnmanagedCallersOnly(CallConvs = new[] { typeof(CallConvCdecl) })]");
        builder.Append("    private static int __PopLua_").Append(first.DispatcherName).AppendLine("(nint state)");
        builder.AppendLine("    {");
        builder.Append("        var __poplua_count = global::PopLua.Binding.Marshaller.Top(state) - ")
            .Append(argumentOffset.ToString(System.Globalization.CultureInfo.InvariantCulture))
            .AppendLine(";");
        builder.AppendLine("        var __poplua_best = -1;");
        builder.AppendLine("        var __poplua_selected = -1;");
        builder.AppendLine("        var __poplua_ambiguous = false;");

        for (var i = 0; i < methods.Count; i++)
        {
            builder.Append("        var __poplua_score_").Append(i).Append(" = ")
                .Append(OverloadCondition(methods[i], argumentOffset))
                .Append(" ? ")
                .Append(OverloadScore(methods[i], argumentOffset))
                .AppendLine(" : -1;");
            builder.Append("        if (__poplua_score_").Append(i).AppendLine(" >= 0)");
            builder.AppendLine("        {");
            builder.Append("            if (__poplua_score_").Append(i).AppendLine(" > __poplua_best)");
            builder.AppendLine("            {");
            builder.Append("                __poplua_best = __poplua_score_").Append(i).AppendLine(";");
            builder.Append("                __poplua_selected = ").Append(i).AppendLine(";");
            builder.AppendLine("                __poplua_ambiguous = false;");
            builder.AppendLine("            }");
            builder.Append("            else if (__poplua_score_").Append(i).AppendLine(" == __poplua_best)");
            builder.AppendLine("            {");
            builder.AppendLine("                __poplua_ambiguous = true;");
            builder.AppendLine("            }");
            builder.AppendLine("        }");
        }

        var owner = NameHelpers.Escape(first.LuaName);
        builder.AppendLine("        if (__poplua_selected < 0 || __poplua_ambiguous)");
        builder.AppendLine("        {");
        builder.Append("            var __poplua_error = new global::PopLua.Exceptions.ScriptException(__poplua_ambiguous ? \"Ambiguous overload for '")
            .Append(owner)
            .Append("'.\" : \"No overload of '")
            .Append(owner)
            .AppendLine("' accepts these arguments.\");");
        builder.AppendLine(first.IsAsync
            ? "            return global::PopLua.Binding.Marshaller.BeginFailedAsync(state, __poplua_error);"
            : "            return global::PopLua.Binding.Marshaller.Error(state, __poplua_error);");
        builder.AppendLine("        }");

        for (var i = 0; i < methods.Count; i++)
            builder.Append("        if (__poplua_selected == ").Append(i).Append(") return __PopLua_")
                .Append(methods[i].BindingName).AppendLine("(state);");

        builder.AppendLine("        throw new global::System.InvalidOperationException(\"Invalid generated overload selection.\");");
        builder.AppendLine("    }");
    }

    private static string OverloadCondition(MethodInfo method, int argumentOffset)
    {
        var parameters = method.Parameters.Where(parameter => !parameter.IsContext).ToArray();
        var variadic = parameters.LastOrDefault() is { } last && IsValueArray(last.Type);
        var fixedParameters = variadic ? parameters.Take(parameters.Length - 1).ToArray() : parameters;
        var required = fixedParameters.Count(parameter => !parameter.HasDefault);
        var conditions = new List<string>
        {
            "__poplua_count >= " + required.ToString(System.Globalization.CultureInfo.InvariantCulture),
        };
        if (!variadic)
            conditions.Add("__poplua_count <= " + fixedParameters.Length.ToString(System.Globalization.CultureInfo.InvariantCulture));

        for (var i = 0; i < fixedParameters.Length; i++)
        {
            var parameter = fixedParameters[i];
            var index = i + 1 + argumentOffset;
            var match = OverloadArgumentMatch(parameter, index);
            if (parameter.HasDefault)
            {
                var omitted = "__poplua_count <= " + i.ToString(System.Globalization.CultureInfo.InvariantCulture);
                if (parameter.AllowsNil)
                    omitted += " || global::PopLua.Binding.Marshaller.IsNil(state, " + index.ToString(System.Globalization.CultureInfo.InvariantCulture) + ")";
                conditions.Add("(" + omitted + " || " + match + ")");
            }
            else
            {
    ...
    private static string OverloadArgumentMatch(ParameterInfo parameter, int index)
    {
        var argument = "state, " + index.ToString(System.Globalization.CultureInfo.InvariantCulture);
        if (parameter.IsText || TypeKey(parameter.Type) == "Value")
            return "!global::PopLua.Binding.Marshaller.IsMissing(" + argument + ")";

        return TypeKey(parameter.Type) switch
        {
            "bool" => "global::PopLua.Binding.Marshaller.IsBoolean(" + argument + ")",
            "int" or "uint" or "long" or "ulong" => "global::PopLua.Binding.Marshaller.IsInteger(" + argument + ")",
            "float" or "double" => "global::PopLua.Binding.Marshaller.IsNumber(" + argument + ")",
            "string" => "global::PopLua.Binding.Marshaller.IsString(" + argument + ")",
            "FunctionRef" => "global::PopLua.Binding.Marshaller.IsFunction(" + argument + ")",
            "Descriptor" or "DescriptorList" or "StringList" =>
                "global::PopLua.Binding.Marshaller.IsTable(" + argument + ")",
            "Userdata" => "global::PopLua.Binding.Marshaller.IsUserdata(" + argument + ", "
                + parameter.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) + ".__PopLua_MetatableName)",
            _ => "true",
        };
    }

    private static string OverloadScore(MethodInfo method, int argumentOffset)
    {
        var parameters = method.Parameters.Where(parameter => !parameter.IsContext).ToArray();
        var variadic = parameters.LastOrDefault() is { } last && IsValueArray(last.Type);
        var fixedParameters = variadic ? parameters.Take(parameters.Length - 1).ToArray() : parameters;
        var baseScore = 1000 - (variadic ? 1 : 0) - fixedParameters.Count(parameter => parameter.HasDefault);
        var score = new StringBuilder(baseScore.ToString(System.Globalization.CultureInfo.InvariantCulture));
        for (var i = 0; i < fixedParameters.Length; i++)
        {
            var parameter = fixedParameters[i];
            var typeScore = parameter.IsText
                ? 5
                : TypeKey(parameter.Type) switch
                {
                    "Value" => 1,
                    "float" or "double" => 20,
                    "Userdata" => 40,
            ...
            score.Append(" + (__poplua_count > ")
                .Append(i.ToString(System.Globalization.CultureInfo.InvariantCulture));
            if (parameter.HasDefault && parameter.AllowsNil)
                score.Append(" && !global::PopLua.Binding.Marshaller.IsNil(state, ")
                    .Append((i + 1 + argumentOffset).ToString(System.Globalization.CultureInfo.InvariantCulture))
                    .Append(')');
            score.Append(" ? ")
                .Append(typeScore.ToString(System.Globalization.CultureInfo.InvariantCulture))
                .Append(" : -2)");
        }
        ...
        IReadOnlyList<ComputedPropertyInfo> computedProperties)
    {
        ...
        foreach (var method in methods)
            module.Functions.Add(BuildFunctionModel(
                method,
                OverloadId(Ids.ModuleMember(moduleName, method.LuaName), method.Overload),
        ...
        IReadOnlyList<OperatorInfo> operators)
    {
        ...
        foreach (var method in methods)
            userdata.Methods.Add(BuildFunctionModel(
                method,
                OverloadId(Ids.UserdataMember(userdataName, method.LuaName), method.Overload),
    ...
    private static string OverloadId(string id, int overload)
        => overload > 0
            ? id + "@" + overload.ToString(System.Globalization.CultureInfo.InvariantCulture)
            : id;
    ...
    private static string ReadParameterExpression(ParameterInfo parameter, int index)
    {
        var read = parameter.IsText
            ? $"global::PopLua.Binding.Marshaller.ReadText(state, {index})"
            : ReadExpression(parameter.Type, index);
        var useDefault = parameter.AllowsNil
            ? $"(global::PopLua.Binding.Marshaller.IsMissing(state, {index}) || global::PopLua.Binding.Marshaller.IsNil(state, {index}))"
            : $"global::PopLua.Binding.Marshaller.IsMissing(state, {index})";
        return parameter.HasDefault
            ? $"{useDefault} ? {parameter.DefaultExpression} : {read}"
            : read;
    }

    private static bool IsSupportedContext(ITypeSymbol type)
        => type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) is
            "global::PopLua.Context.ScriptContext" or "global::System.Threading.CancellationToken";

    private static string ContextExpression(ITypeSymbol type, string context)
    ...
    private static string DefaultExpression(IParameterSymbol parameter)
    {
        ...
        return parameter.Type.SpecialType switch
        {
            ...
            SpecialType.System_String => SymbolDisplay.FormatLiteral((string)parameter.ExplicitDefaultValue, quote: true),
            SpecialType.System_Int32 => ((int)parameter.ExplicitDefaultValue).ToString(System.Globalization.CultureInfo.InvariantCulture),
            SpecialType.System_UInt32 => ((uint)parameter.ExplicitDefaultValue).ToString(System.Globalization.CultureInfo.InvariantCulture) + "U",
            SpecialType.System_Int64 => ((long)parameter.ExplicitDefaultValue).ToString(System.Globalization.CultureInfo.InvariantCulture) + "L",
            SpecialType.System_UInt64 => ((ulong)parameter.ExplicitDefaultValue).ToString(System.Globalization.CultureInfo.InvariantCulture) + "UL",
            SpecialType.System_Single => ((float)parameter.ExplicitDefaultValue).ToString("R", System.Globalization.CultureInfo.InvariantCulture) + "F",
            SpecialType.System_Double => ((double)parameter.ExplicitDefaultValue).ToString("R", System.Globalization.CultureInfo.InvariantCulture),
            _ => "default",
        };
    }
