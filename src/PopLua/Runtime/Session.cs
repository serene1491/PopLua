...
public sealed class Session : IAsyncDisposable
{
    ...
    private static readonly byte[] RequireWrapper = """
        ...
        return function(name)
            ...
            if lua_pcall ~= nil then
                ...
                if has_failed() then
                    finish(name, "failed", "module execution failed")
                    return nil
                end
            end
    ...
    private readonly List<Load> _loadingModules = [];
    private readonly Stopwatch _activeClock = new();
    private readonly Stopwatch _suspendedClock = new();
    private readonly Stopwatch _wallClock = new();
    private readonly CancellationTokenSource _disposeCts = new();
    private readonly string _stepPrefix = Guid.NewGuid().ToString("N");
    ...
    /// <summary>
    /// Gets the logical host run shared by execution steps in this session.
    /// </summary>
    ...
    public ValueTask<Result> Call(string global, params Value[] args)
        => Call(global, CancellationToken.None, args);

    /// <summary>
    /// Calls a global Lua function with cancellation and the provided arguments.
    /// </summary>
    /// <param name="global">Name of the global Lua function to call.</param>
    /// <param name="ct">Cancellation token that terminates the call and async bridge waits.</param>
    /// <param name="args">Arguments pushed to Lua before the function call.</param>
    /// <returns>A result containing the first Lua return value, or an execution error.</returns>
    /// <exception cref="ArgumentException">Thrown when <paramref name="global"/> is <see langword="null"/>, empty, or whitespace.</exception>
    public ValueTask<Result> Call(string global, CancellationToken ct, params Value[] args)
    {
        ThrowIfDisposed();
        ArgumentException.ThrowIfNullOrWhiteSpace(global);

        return new ValueTask<Result>(StartExecution(ctx => ExecuteCallAsync(ctx, global, args), ct));
    }
    ...
    /// <summary>
    /// Calls a global Lua function with cancellation and converts its first result.
    /// </summary>
    /// <typeparam name="T">The expected C# result type.</typeparam>
    /// <param name="global">Name of the global Lua function to call.</param>
    /// <param name="ct">Cancellation token that terminates the call and async bridge waits.</param>
    /// <param name="args">Arguments pushed to Lua before the function call.</param>
    /// <returns>A typed result containing the converted first Lua value, or an execution error.</returns>
    public async ValueTask<Result<T>> Call<T>(
        string global,
        CancellationToken ct,
        params Value[] args)
        => ToTypedResult<T>(await Call(global, ct, args).ConfigureAwait(false));
    ...
    private Task<Result> ExecuteChunkAsync(ScriptContext ctx, Chunk chunk)
    {
        var name = chunk.Name ?? "chunk";
        return ExecuteAsync(ctx, name, StepKind.Run, execution =>
        {
            var loadError = Load(execution.Thread, chunk.Bytes.Span, chunk.Name ?? "chunk");
            if (loadError is not null)
                return Task.FromResult(Fail(ctx, loadError, execution));

            return ResumeUntilCompleteAsync(execution, args: 0);
        });
    }

    private Task<Result> ExecuteCallAsync(ScriptContext ctx, string global, Value[] args)
        => ExecuteCallableAsync(ctx, $"call:{global}", state => PushGlobal(state, global), args);
    ...
    private Task<Result> ExecuteFunctionRefAsync(ScriptContext ctx, int registryReference, Value[] args)
        => ExecuteCallableAsync(
            ctx,
            "call:function-ref",
            state => NativeApi.RawGetI(state, NativeApi.RegistryIndex, registryReference),
            args);

    private Task<Result> ExecuteCallableAsync(
        ScriptContext ctx,
        string name,
        Action<nint> push,
        Value[] args)
        => ExecuteAsync(ctx, name, StepKind.Call, execution =>
        {
            push(execution.Thread);
            var stack = new StateStack(new StateHandle(execution.Thread));
            var actual = stack.TypeOf(-1);
            if (actual != NativeType.Function)
            {
                return Task.FromResult(Fail(
                    ctx,
                    new NativeTypeException("function", ToValueKind(actual)),
                    execution));
            }
            ...
            return ResumeUntilCompleteAsync(execution, args.Length);
        });
    ...
    private async Task<Result> ExecuteAsync(
        ...
        string name,
        StepKind kind,
        Func<Execution, Task<Result>> execute)
    {
        var step = NewStep(kind, name);
        _runtime.Observers.Started(ctx, step);

        var execution = CreateExecution(ctx, name, step);
        try
        {
            return await execute(execution).ConfigureAwait(false);
        }
        finally
        {
    ...
    private Execution CreateExecution(ScriptContext ctx, string? chunkName, Step step)
    {
        _instructions = 0;
        _callDepth = 0;
        ...
        return new Execution(ctx, thread, threadRef, chunkName, step);
    }

    private void DisposeExecution(Execution execution)
    {
        _activeClock.Stop();
        _suspendedClock.Stop();
        _wallClock.Stop();
    ...
    private async Task<Result> ResumeUntilCompleteAsync(Execution execution, int args)
    {
        while (true)
        {
            ...
            var duration = _wallClock.Elapsed;
            var active = _activeClock.Elapsed;
            var suspended = _suspendedClock.Elapsed;
            var metrics = new Metrics(
                ...
                _suspensions,
                _instructions,
                ToLong(_lua.Allocator?.PeakBytes),
                _maxCallDepthObserved);
            _runtime.Observers.Completed(execution.Context, execution.Step, in metrics);
    ...
    private ScriptContext CreateContext(CancellationToken cancellation, out CancellationTokenSource linkedCts)
    {
        ...
        return ScriptContext.Create(Sandbox, Identity, RunInfo, Services, linkedCts.Token);
    }
    ...
    private Step NewStep(StepKind kind, string name, string? parent = null)
        => new($"{_stepPrefix}:{++_nextStep}", kind, name, parent);

    private Result Fail(ScriptContext ctx, RuntimeException error, Execution execution)
    {
        error = Enrich(error, execution);
        new StateStack(new StateHandle(execution.Thread)).SetTop(0);
        FailOutstandingLoads(ctx, error);

        if (error is QuotaException quota)
            _runtime.Observers.Quota(ctx, execution.Step, quota.Kind);

        if (error is SandboxException sandbox)
            _runtime.Observers.Denied(ctx, execution.Step, sandbox.Cap);

        _runtime.Observers.Failed(ctx, execution.Step, error);
        return Result.Failure(error);
    }

    private void FailOutstandingLoads(ScriptContext ctx, RuntimeException error)
    {
        for (var i = _loadingModules.Count - 1; i >= 0; i--)
            _runtime.Observers.LoadFailed(ctx, _loadingModules[i], error);
    ...
    private static int RequireBeginCallback(nint state)
    {
        try
        {
            ...
            var existingIndex = session._loadingModules.FindIndex(load =>
                string.Equals(load.Name, normalized, StringComparison.Ordinal));
            if (existingIndex >= 0)
                return PushRequireFailure(stack, "cyclic module load: " + session.FormatModuleLoadCycle(existingIndex, normalized));

            var parent = session._loadingModules.Count > 0
                ? session._loadingModules[^1].Step.Id
                : session._activeStep?.Id;
            var caller = session._loadingModules.Count > 0
                ? session._loadingModules[^1].Name
                : null;
            var load = new Load(
                normalized,
                caller,
                session._loadingModules.Count + 1,
                session.NewStep(StepKind.Load, normalized, parent));
            session._loadingModules.Add(load);
            var ctx = Session.CurrentContext ?? ScriptContext.Create(run: session.RunInfo);
            if (session._runtime.Observers.Any)
                session.ObserveDuringResume(() => session._runtime.Observers.Loading(ctx, load));
    ...
    private static int RequireResolveCallback(nint state)
    {
        try
        {
            ...
            if (session._loadingModules.Count == 0
                || !string.Equals(session._loadingModules[^1].Name, moduleName, StringComparison.Ordinal))
            {
                return PushRequireMissing(stack, $"module load state unavailable: {moduleName}");
            }

            var load = session._loadingModules[^1];
            Chunk? resolved;
            try
            {
                resolved = session._moduleResolver(
                    Session.CurrentContext ?? ScriptContext.Create(run: session.RunInfo),
                    load);
            }
            catch (Exception ex)
            {
    ...
    private static int RequireFinishCallback(nint state)
    {
        try
        {
            ...
            {
                var stack = new StateStack(new StateHandle(state));
                if (stack.TypeOf(1) == NativeType.String && stack.TypeOf(2) == NativeType.String)
                {
                    var message = stack.TypeOf(3) == NativeType.String ? stack.ReadString(3) : null;
                    session.FinishModuleLoad(stack.ReadString(1), stack.ReadString(2), message);
                }
            }
    ...
    private string FormatModuleLoadCycle(int startIndex, string repeated)
    {
        ...
        for (var i = startIndex; i < _loadingModules.Count; i++)
        {
            ...
            builder.Append(_loadingModules[i].Name);
        }
    ...
    private void FinishModuleLoad(string name, string status, string? message)
    {
        for (var i = _loadingModules.Count - 1; i >= 0; i--)
        {
            if (string.Equals(_loadingModules[i].Name, name, StringComparison.Ordinal))
            {
                var load = _loadingModules[i];
                _loadingModules.RemoveAt(i);
                var ctx = Session.CurrentContext ?? ScriptContext.Create(run: RunInfo);
                if (string.Equals(status, "failed", StringComparison.Ordinal))
                {
                    if (_runtime.Observers.Any)
                    {
                        ObserveDuringResume(() => _runtime.Observers.LoadFailed(
                                ctx,
                                load,
                                new ScriptException(message ?? $"module load failed: {name}")));
                    }
                }
                else
                {
                    if (_runtime.Observers.Any)
                    {
                        ObserveDuringResume(() => _runtime.Observers.Loaded(
                                ctx,
                                load,
                                cached: string.Equals(status, "cached", StringComparison.Ordinal)));
                    }
                }
    ...
    private void ObserveDuringResume(Action notify)
    {
        var active = _activeClock.IsRunning;
        if (active)
            _activeClock.Stop();
    ...
    {
        internal Execution(ScriptContext context, nint thread, int threadRegistryRef, string? chunkName, Step step)
        {
            Context = context;
            Thread = thread;
