...
public static class Marshaller
{
    ...
    private static readonly byte[] ToStringMetamethod = "__tostring\0"u8.ToArray();
    ...
    /// <summary>
    /// Reads display text from a Lua string, scalar, or value with a
    /// <c>__tostring</c> metamethod.
    /// </summary>
    public static string ReadText(nint state, int index)
    {
        var absolute = NativeApi.AbsIndex(state, index);
        return NativeApi.Type(state, absolute) switch
        {
            NativeType.String => ReadString(state, absolute),
            NativeType.Number when NativeApi.IsInteger(state, absolute) =>
                ReadLong(state, absolute).ToString(CultureInfo.InvariantCulture),
            NativeType.Number => ReadDouble(state, absolute).ToString("G17", CultureInfo.InvariantCulture),
            NativeType.Boolean => ReadBool(state, absolute) ? "true" : "false",
            NativeType.Nil => "nil",
            _ => ReadTextMetamethod(state, absolute),
        };
    }

    /// <summary>
    /// Reports whether a Lua argument was omitted from the call.
    /// </summary>
    public static bool IsMissing(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.None;

    /// <summary>
    /// Reports whether a Lua argument is explicitly <c>nil</c>.
    /// </summary>
    public static bool IsNil(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Nil;

    /// <summary>
    /// Reports whether a Lua argument is a boolean.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is a boolean.</returns>
    public static bool IsBoolean(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Boolean;

    /// <summary>
    /// Reports whether a Lua argument is an integer number.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is represented as a Lua integer.</returns>
    public static bool IsInteger(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Number && NativeApi.IsInteger(state, index);

    /// <summary>
    /// Reports whether a Lua argument is a number.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is a Lua number.</returns>
    public static bool IsNumber(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Number;

    /// <summary>
    /// Reports whether a Lua argument is a string.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is a Lua string.</returns>
    public static bool IsString(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.String;

    /// <summary>
    /// Reports whether a Lua argument is a table.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is a Lua table.</returns>
    public static bool IsTable(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Table;

    /// <summary>
    /// Reports whether a Lua argument is a function.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <returns><see langword="true"/> when the argument is a Lua function.</returns>
    public static bool IsFunction(nint state, int index)
        => NativeApi.Type(state, index) == NativeType.Function;

    /// <summary>
    /// Reports whether a Lua argument is userdata with the expected generated metatable.
    /// </summary>
    /// <param name="state">Native Lua state pointer supplied by generated callbacks.</param>
    /// <param name="index">Lua stack index to inspect.</param>
    /// <param name="metatableName">Expected generated userdata metatable name.</param>
    /// <returns><see langword="true"/> when the argument has the expected metatable.</returns>
    public static unsafe bool IsUserdata(nint state, int index, string metatableName)
    {
        ...
        fixed (byte* namePtr = buffer[..length])
            return NativeApi.TestUserData(state, index, namePtr) != 0;
    }
    ...
    private static unsafe string ReadTextMetamethod(nint state, int index)
    {
        var top = NativeApi.GetTop(state);
        try
        {
            if (NativeApi.GetMetaTable(state, index) == 0)
                throw new NativeTypeException("text", ToValueKind(NativeApi.Type(state, index)));

            fixed (byte* key = ToStringMetamethod)
            {
                if (NativeApi.GetField(state, -1, key) != NativeType.Function)
                    throw new NativeTypeException("text", ToValueKind(NativeApi.Type(state, index)));
            }

            NativeApi.PushValue(state, index);
            var status = NativeApi.PCall(state, 1, 1, errorFunction: 0, context: 0, continuation: 0);
            if (status != 0)
                throw new ScriptException(ReadErrorMessage(state));
            if (NativeApi.Type(state, -1) != NativeType.String)
                throw new ScriptException("__tostring must return a string");

            return ReadString(state, -1);
        }
        finally
        {
            NativeApi.SetTop(state, top);
