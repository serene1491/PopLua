...
`[Fn(CountWait = true)]` for async host work whose suspended
...
when Lua provides it. Add one or more observers with
`EngineBuilder.Observe(...)` to receive correlated `RunInfo` and `Step` events,
controlled loads, quota and capability blocks, and final active/suspended
metrics. Observer failures do not fail the script.
