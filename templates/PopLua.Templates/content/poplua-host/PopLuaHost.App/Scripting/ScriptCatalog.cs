...
internal sealed class ScriptCatalog
{
    ...
    public Chunk? ResolveModule(ScriptContext context, Load load)
        => _modules.TryGetValue(load.Name, out var source)
            ? Chunk.Code(source, $"{context.Identity.Id}:{load.Name}.lua")
            : null;
}
