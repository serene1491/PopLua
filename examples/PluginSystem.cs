...
[Module("log", Cap = PluginCaps.Log)]
public partial class LogModule(IPluginLog log, PluginScope scope)
{
    [Fn("info")]
    public void Info(string message) => log.Write(scope.PluginId, "info", message);
}

[Module("state", Cap = PluginCaps.State)]
public partial class StateModule(PluginState state)
{
    [Fn("get")]
    public long Get(string key) => state.Get(key);

    [Fn("set")]
    public void Set(string key, long value) => state.Set(key, value);
}
