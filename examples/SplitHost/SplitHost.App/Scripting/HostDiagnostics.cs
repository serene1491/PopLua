...
internal sealed class HostDiagnostics : IRunObserver
{
    ...
    public void Completed(ScriptContext context, Step step, in Metrics metrics)
        => _events.Add($"completed {step.Id}: active={metrics.Active.TotalMilliseconds:F1}ms, suspended={metrics.Suspended.TotalMilliseconds:F1}ms, instructions={metrics.Instructions}");
