...
work="$(mktemp -d /tmp/poplua-rc2-smoke.XXXXXX)"
...
var lua = Engine.Create(b => b.Require((ctx, load) =>
    load.Name == "util" ? Chunk.Code("return { answer = 42 }", "module:util.lua") : null));
...
var lua = Engine.Create(b => b
    .Module<HostModule>()
    .Require((ctx, load) => load.Name == "util" ? Chunk.Code("return { tag = 'ok' }", "module:util.lua") : null));
