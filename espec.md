...
```xml
<PackageReference Include="PopLua" Version="1.0.0-rc.2" />
```
...
- `[Fn]` methods may share a Lua name when their Lua-visible parameter shapes
  differ. Resolution uses arity and Lua types before host invocation.
- One overload group must be entirely synchronous or entirely asynchronous.
  Indistinguishable CLR shapes such as `int`/`long` are rejected.
...
public sealed class EngineBuilder
{
    ...
    public EngineBuilder Observe(IRunObserver observer);
    public EngineBuilder Allocator(AllocatorOptions options);
    public EngineBuilder Require(ModuleResolver resolver);
}

public delegate Chunk? ModuleResolver(ScriptContext context, Load load);
```
...
- any `[Userdata]` type
- `ValueTask` and `ValueTask<T>` when `[Fn]` is set
...
- one generated binding callback per Lua function, with a generated dispatcher
  and private candidate callbacks when it has overloads;
...
| `PLUA012` | `CountWait` on a method that does not return `ValueTask` or `ValueTask<T>` |
| `PLUA015` | overloads have the same Lua-visible parameter shape |
| `PLUA016` | overload group mixes synchronous and asynchronous methods |
...
Overload entries append a stable one-based suffix (`@1`, `@2`, ...) and include
their ordinal as function metadata.
