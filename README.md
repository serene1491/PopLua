...
The resolver receives a validated `Load` containing the strict dot-separated
name, caller, depth, and correlated step, then maps it to a host-approved
`Chunk`. PopLua does not use `package.path` or provide filesystem loading by
default.
...
Generated `[Fn]` methods may share a Lua name when Lua can distinguish their
parameter shapes:

```csharp
[Fn("find")]
public static Item Find(long id) => repository.Find(id);

[Fn("find")]
public static Item Find(string name) => repository.Find(name);
```

PopLua resolves the overload from Lua arity and value types before invoking host
code. It rejects indistinguishable shapes, such as `int` versus `long`, and does
not use declaration order as a fallback.
